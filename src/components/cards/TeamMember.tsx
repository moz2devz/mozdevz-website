...
const TeamMember: React.FC<TeamMemberProps> = (props) => {
  return (
    <div className="w-full max-w-[500px] sm:max-w-[300px]">
      <div className="relative group mb-5">
        <div className="overflow-hidden rounded-md origin-center">
          <Image
            src={props.img}
            alt={props.name}
            className="w-full hover:scale-125 transition-all ease-in-out duration-700"
          />
        </div>
          ...
          <div className="flex gap-10 justify-center translate-y-10 group-hover:translate-y-0 transition-transform duration-[750ms]">
            {props.linkedin && (
              <a href={props.linkedin}>
                <LinkedinLogo size={20} color="#fff" />
              </a>
            )}

            {props.twitter && (
              <a href={props.twitter}>
                <TwitterLogo size={20} color="#fff" />
              </a>
            )}
      ...
      <span className="text-[0.8rem] text-gray-500 uppercase mt-3">
        {props.role}
      </span>
    </div>
  )
